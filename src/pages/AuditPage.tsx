...
export default function AuditPage() {
  ...
  useEffect(() => {
    if (!user) return
    Promise.all([
      supabase.from('api_keys').select('service').eq('user_id', user.id).eq('service', 'claude').maybeSingle(),
      supabase.from('api_keys').select('service').eq('user_id', user.id).eq('service', 'openrouter').maybeSingle(),
    ]).then(([claude, openrouter]) => setHasClaudeKey(!!(claude.data || openrouter.data)))
  }, [user])
